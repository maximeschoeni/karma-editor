	...
	createItem: function() {
		...
		var item = {
			...
			findDescendants: function(filter) {
				...
				// }
				// return [];
				var descendants = [this].filter(filter);

				this.zone.items.filter(filter).forEach(function(subItem) {
					descendants = descendants.concat(subItem.findDescendants(filter));
				});
				return descendants;
			},
			getRoot: function() {
			...
			getSelection: function() {
				var selection = {
					...
					getInbetween: function() {
						var item = this.getFirst().getNextSibling();
						while (item && item !== this.getLast()) {
							if (!item.selected) {
								return item;
							}
			...
			mousedown: function(event) {
				if (item.selected) {
					if (event.metaKey || event.controlKey) {
						...
						selection.rect.left = event.clientX - offsetX;
						selection.rect.top = event.clientY - offsetY;
						var itemUnder = selection.findItemUnder(0, 0);
