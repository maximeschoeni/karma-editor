...
// KarmaFields.managers.field = function(resource, path, history, selection, parent) {
	...
	var manager = {
		...
		init: function(resource, args) {
			...
			this.name = resource.field || resource.name;
		...
		getValue: function() {
			var value = args.history.read(this.output);
			if (value === undefined) {
				value = args.history.read(this.input);
			}
			return value;
		},
		setValue: function(value, noBreakHistory, noEditOthers) {
			...
			// 	// console.log("breakHistory");
			// 	args.history.breakHistory();
			// }
			if (this.hasHistory && !noBreakHistory) {
				// console.log("breakHistory");
				args.history.maybeBreakHistory(this.output);
			}
		...
		fetchOptions: function(custom) {
			if (resource.driver && resource.key) {
				...
				var params = {};
				if (args.history) {
					var filters = args.history.read(["filters"]);
