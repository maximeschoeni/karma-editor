		...
		$karma_fields->register_middleware('posts', array(
			...
			'fields' => array(
				...
				'postmeta' => array(
					...
					'update' => function($uri, $key, $value, $args) {
						// $args = array(
				...
				// ),
				'file' => array(
					'update' => function($uri, $key, $value, $resource, $args) {

						$attachment_id = $value->id;

						$id = apply_filters('karma_fields_posts_uri', $uri);

						$prev = intval(get_post_meta($id, $key, true));

						if ($attachment_id != $prev) {

							if (isset($resource['cache']) && $resource['cache']) {

								if ($resource['cache'] === true) {

									$cache = $key.".{$resource['type']}";
							...
							if ($attachment_id) {
								...
								if (isset($cache) && $prev) {
									...
									do_action('karma_cache_posts_add_dependancy', $attachment_id, "$uri/$cache");
								...
								delete_post_meta($id, $key, $attachment_id);
					...
					'get' => function($id, $key) {
						...
						$img_id = get_post_meta($id, $key, true);
						...
						$output['thumb'] = $thumb_src_data[0];
						$output['thumb_width'] = $thumb_src_data[1];
						$output['thumb_height'] = $thumb_src_data[2];
