...
Class Karma_Posts_Fields {
			...
			if (isset($_REQUEST[$nonce]) && wp_verify_nonce($_POST[$nonce], $action)) {
				...
				if (isset($_REQUEST['karma-fields-items']) && is_array($_REQUEST['karma-fields-items'])) {
					...
					foreach ($_REQUEST['karma-fields-items'] as $item) {
						...
						$karma_fields->update_item('posts', $item, $post_id, null);
	...
	//
	// 	wp_register_script('build', KARMA_FIELDS_URL . '/js/build-v4.js', array(), $this->version, false);
	...
	//
	// 	$this->update_dependancies($post, $cache);
	//
	// 	$sections = $this->get_sections();
	// 	$sections = $this->filter_fields($sections, 'post_type', $post->post_type, false);
	//
	...
	// //  * update_dependancies
	// //  */
	// // public function update_dependancies($post, $cache) {
	// // 	global $wpdb;
	// //
	// // 	$dependencies = $wpdb->get_col($wpdb->prepare("SELECT post_id WHERE meta_key = 'karma-relation' AND meta_value = %d", $post->ID));
	// //
	// // 	if ($dependencies) {
	// //
	// // 		$sections = $this->get_sections();
	// //
	// // 		$fields = $this->filter_fields($sections, 'dependancy', $post->post_type, true);
	// //
	// // 		foreach ($dependencies as $id) {
	// //
	// // 			foreach ($fields as $field) {
	// //
	// // 				$cache->delete($id, $field['key']);
	// //
	// // 			}
	// //
	// // 		}
	...
	// //  * update_dependancies
	// //  */
	// // public function update_dependancies_v2($post, $cache) {
	// // 	global $wpdb;
	// //
	// // 	$sections = $this->get_sections();
	// //
	// // 	$fields = $this->filter_fields($sections, 'field', 'relation');
	// //
	// // 	if ($fields) {
	// //
	// // 		$keys = array();
	...
	// // 		foreach ($fields as $field) {
	// //
	// // 			$keys[] = esc_sql($field['key']);
	// //
	// // 		}
	// //
	// // 		$fields_sql = implode("','", $keys)
	// //
	// // 		$dependencies = $wpdb->get_col($wpdb->prepare("SELECT post_id WHERE meta_key IN ('$keys') AND meta_value = %d", $post->ID));
	// //
	// // 		if ($dependencies) {
	// //
	// // 			foreach ($dependencies as $id) {
	...
	//
	// 	$fields = $this->filter_fields($sections, 'field', 'relation');
	// 	$fields = $this->filter_fields($fields, 'post_type', $post->post_type);
	//
	// 	foreach ($fields as $field) {
	//
	// 		$dependencies = $wpdb->get_col($wpdb->prepare("SELECT post_id WHERE meta_key = %s AND meta_value = %d", $field['key'], $post->ID));
	//
	// 		foreach ($dependencies as $id) {
	//
	// 			$cache->delete($id, $field['key']);
	...
	// // 	foreach ($fields as $field) {
	// //
	// // 		if (isset($field['object']) && $field['object'] === 'postmeta') {
	// //
	// // 			$relation_ids = array_merge($relation_ids, get_post_meta($post->ID, $field['key']));
	// //
	// // 		}
	// //
	// // 	}
	...
	// //
	// // 	$dependency_ids = $this->delta_postmeta($post->ID, 'karma-relation', $relation_ids);
	// //
	// // }
	//
	// /**
	...
	//
	// 				$this->delta_postmeta($post_id, $field['key'], $value);
	//
	// 			} else {
	//
	...
	//
	// /**
	//  * update multiple post meta values with single key
	//  */
	// // public function update_multi_postmeta($post_id, $key, $values) {
	// public function delta_postmeta($post_id, $key, $values) {
	//
	...
	// 	}
	//
	// 	if ($field['field'] === 'relation' && empty($field['object'])) {
	//
	// 		if (empty($field['object'])) {
	//
	// 			$field['object'] = 'postmeta';
	//
	// 		}
	//
	// 		if (empty($field['dependancy'])) {
	//
	// 			$field['dependancy'] = $field['post_type'];
	//
	// 		}
	//
	// 	}
	...
	//  * @hook add_meta_boxes
	//  */
	// public function find_field($fields, $key, $value, $recursive = true) {
	//
	// 	foreach ($fields as $field) {
	//
	...
	// 			return $field;
	//
	// 		} else if ($recursive && isset($field['children'])) {
	//
	...
	//  * filter fields
	//  */
	// public function filter_fields($fields, $key, $value, $recursive = true) {
	//
	...
	// 		}
	//
	// 		if ($recursive && isset($field['children'])) {
	//
	...
	// 	$sections = $this->get_sections();
	//
	// 	$sections = $this->filter_fields($sections, 'post_type', $post_type, false);
	//
	...
	//
	//
	// // public function register_parasite($args) {
	// //
	// // 	add_filter('karma_fields_parasites', $args['callback']);
	// //
	// //
	// // }
