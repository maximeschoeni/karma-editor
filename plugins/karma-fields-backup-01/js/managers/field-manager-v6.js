...
KarmaFields.managers.field = function(resource, post, middleware, history, parent) {
	var manager = {
		...
		fetchOptions: function(filters) {
			if (resource.key) {
				var path = KarmaFields.fetchURL+"/"+(resource.driver || middleware)+"/"+resource.method+"/"+resource.key;
				if (this.table) {
					var params = [];
					for (var key in this.table.filters) {
						if (this.table.filters[key]) {
				...
				if (!KarmaFields.fetchCache[path]) {
					KarmaFields.fetchCache[path] = fetch(path, {
						cache: "default" // force-cache
					}).then(function(response) {
						return response.json();
					});
				}
				return KarmaFields.fetchCache[path];
			} else {
				return Promise.resolve();
			}
    }
