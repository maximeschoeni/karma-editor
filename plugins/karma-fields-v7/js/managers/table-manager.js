    ...
    // ppp: resource.ppp || 10,
		...
		request: function() {
			...
			if (resource.driver) {
				...
				KarmaFields.Transfer.query(resource.driver, params).then(function(results) {
		...
		sync: function() {
			...
			var options = history.read(["options"]);
			var filters = history.read(["filters"]);
			Object.merge(params, filters);
			Object.merge(params, options);
			// var options = history.read(["options"]);
			// var filters = history.read(["filters"]);
			...
			KarmaFields.Transfer.update(resource.driver, params).then(function(results) {
